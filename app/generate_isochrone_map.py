...
def OLD_isochrone_generator(isoc_area, borders, point_locations, base_map, isoc_colors=[], point_color="grey", center_name="", buffer_zone=0, buffer_interval=0, buffer_color="black", map_color="grey", background_color="white", line_color="white"):
...
def isochrone_generator(isoc_area, borders, point_locations, base_map, isoc_colors=[], point_color="grey", center_name="", buffer_zone=0, buffer_interval=0, buffer_color="black", map_color="grey", background_color="white", line_color="white"):
    # Isokroni-tiedoston GeoDataFrame. Muuten Suomen viralliseen projektiomuotoon.
    isoc_area = isoc_area.to_crs(epsg=4326) 
    # Luodaan alueelle raja.
    covered_area = isoc_area.geometry.unary_union 
    area_borders = borders.to_crs(epsg=4326)
    # Poistetaan kaikki kohteet rajojen ulkopuolella.
    point_locations = point_locations.to_crs(epsg=4326) 
    point_locations = point_locations[point_locations.geometry.within(covered_area)]
    ...
    # Map vetoaa pohjakarttaan. Kartta piirretään ja sitä voi käyttää sen jälkeen pohjana muille elementeille.
    map = folium.Map(location=[latitude, longitude])

    # Piirretään isokroni-alue ja karttaan tulevat kohteet ja
    # muutetaan isokronit .json muotoon.
    geo_json = isoc_area.to_json()
    folium.GeoJson(
            data=geo_json,
        ).add_to(map)

    # Save the map as a html file.
    filename = "isokroni-kartta.html"
    map.save(filename)
